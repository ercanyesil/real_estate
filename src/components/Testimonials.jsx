...
const Testimonials = () => {
    ...
    const renderStars = (rating) => {
        const stars = [];
        for (let i = 0; i < 5; i++) {
            stars.push(<div key={i} className={`inline ${i < rating ? "text-yellow-400" : "text-gray-300"}`} />);
        }
        return stars;
    };
    ...
    const renderIndicators = () => {
      ...
      const cardStyle = {
        ...
        borderRadius: "30px",
        padding: '50px',
        boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)', // Apply custom box-shadow here
      };

    return (
        <section className="bg-white py-16">
            <div className="container mx-auto flex flex-col md:flex-row items-center justify-between px-[95px]">
               ...
               <div className="md:w-1/2 relative flex">
                    <div className="absolute -top-8 -left-4 w-16 h-16 rounded-full bg-gradient-to-b from-gradient-start to-gradient-end"></div>
                    <div className="relative bg-white z-10 " style={cardStyle}>
                       {/* Quote Marks */}
                        <div className="text-yellow-400 text-2xl mb-2">
                         ...
                         {/* Testimonial Text */}
                       <p className="text-[19.4px] text-gray-700 mb-8 font-poppins font-medium leading-[29.1px] text-left underline-from-font decoration-skip-ink-none">{currentTestimonial.text}</p>
